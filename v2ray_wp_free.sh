...
#使用方法 
#wget -N --no-check-certificate -q -O install.sh "https://raw.githubusercontent.com/jth445600/hello-world/master/v2ray_wp_free" && chmod +x install.sh && bash install.sh
...
last_config() {
	sed -i "s:root  /home/wwwroot/3DCEList:root  /usr/share/nginx/html:g" /etc/nginx/conf/conf.d/v2ray.conf
	sed -i 's/index index.html index.htm/index index.php index.html index.htm/g' /etc/nginx/conf/conf.d/v2ray.conf
	sed -i '11a location ~ \.php$ {' /etc/nginx/conf/conf.d/v2ray.conf
	sed -i '12a fastcgi_pass 127.0.0.1:9000;' /etc/nginx/conf/conf.d/v2ray.conf 
	sed -i '13a fastcgi_param SCRIPT_FILENAME /usr/share/nginx/html$fastcgi_script_name;' /etc/nginx/conf/conf.d/v2ray.conf
	sed -i '14a fastcgi_index index.php;' /etc/nginx/conf/conf.d/v2ray.conf 
...
menu() {
    ...
    echo -e "—————————————— 安装向导 ——————————————"""
  	echo -e "${Green}1.${Font}  安装 V2Ray (Nginx+ws+tls+wordpress+节点爬虫)"
